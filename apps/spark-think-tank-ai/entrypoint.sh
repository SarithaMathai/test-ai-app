#!/bin/sh
# Entrypoint for spark-think-tank-ai FastAPI service.
#
...
echo "Starting spark-think-tank-ai — workers=${GUNICORN_WORKERS}, port=${APP_PORT}, config_dir=${APP_CONFIG_DIR}"

exec .venv/bin/gunicorn spark_think_tank_ai.main:app \
