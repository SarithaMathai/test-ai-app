...
def _load_eval_history(limit: int = 10) -> list[dict]:
    ...
    if db is None:
        return []
    try:
        docs = list(db[_EXTENDED_EVAL_COL].find({}).sort("created_at", -1).limit(limit))
        return docs
    except Exception:
        return []
...
def _render_overview():
    ...
    with col1:
        if by_status:
            st.bar_chart({status: count for status, count in by_status.items()})
        st.caption("Mappings by status")

    with col2:
        if by_tier:
            st.bar_chart({tier: count for tier, count in by_tier.items()})
