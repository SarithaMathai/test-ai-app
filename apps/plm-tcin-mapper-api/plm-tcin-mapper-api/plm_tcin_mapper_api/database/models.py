...
class Mapping(BaseModel):
    ...
    color_possible_values: list[ColorCandidate] = Field(default_factory=list)
...
class SignalAccuracy(BaseModel):
    ...
    confidence_by_tier: dict[str, int] = Field(default_factory=dict)
...
class DepartmentMetrics(BaseModel):
    ...
    correction_rate: float
    avg_confidence: float
    by_match_round: dict[str, int] = Field(default_factory=dict)
...
class ExtendedEvalRun(BaseModel):
    ...
    per_signal_accuracy: dict[str, SignalAccuracy] = Field(default_factory=dict)
    per_department_metrics: list[DepartmentMetrics] = Field(default_factory=list)
...
class ShadowComparisonResult(BaseModel):
    ...
    metric_comparisons: list[ShadowMetricComparison] = Field(default_factory=list)
...
class ThresholdProposal(BaseModel):
    ...
    estimated_impact: list[ImpactEstimate] = Field(default_factory=list)
