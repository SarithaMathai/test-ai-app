...
def _get_pids_to_match(
    db: Any, cfg: Any, force: bool, department: str | None, unmatched_only: bool = False
) -> list[str]:
    ...
    all_pids: list[str] = tcin_col.distinct("pid", filt)

    # Resolve effective scope:
    # • force=True overrides unmatched_only → all PIDs (legacy behaviour)
    # • unmatched_only=True → only PIDs with no mapping OR explicit NO_MATCH
    # • default (both False) → all PIDs (re-process everything)
    if not force and unmatched_only:
        in_mappings: set[str] = set(mapping_col.distinct("pid"))
        no_match_pids: set[str] = set(mapping_col.distinct("pid", {"status": "NO_MATCH"}))
        # Keep PIDs that either have never been mapped, or are explicitly NO_MATCH
        all_pids_set = set(all_pids)
        return sorted((all_pids_set - in_mappings) | (all_pids_set & no_match_pids))
...
def match_pid(
    ...
    from plm_tcin_mapper_api.matching.deterministic import match_pid_records

    raw_results = match_pid_records(tcin_records, variation_records, cfg)

    if use_llm and raw_results:
        try:
            from plm_tcin_mapper_api.llm.disambiguator import disambiguate_low_confidence
