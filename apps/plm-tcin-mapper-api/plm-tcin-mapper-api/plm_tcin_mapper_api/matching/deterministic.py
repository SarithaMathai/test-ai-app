...
from plm_tcin_mapper_api.matching.scorer import (
    HIGH_CONF_THRESHOLD,
    build_score_matrix,
    candidate_list,
    color_score,
)
from plm_tcin_mapper_api.matching.size_normalizer import best_size_match
...
def match_pid_records(
    ...
    for tcin_rec in tcin_records:
        tcin_by_color.setdefault(tcin_rec.color_name, []).append(tcin_rec)

    var_by_impression: dict[str, list[VariationRecord]] = {}
    for var_rec in variation_records:
        var_by_impression.setdefault(var_rec.impression_name, []).append(var_rec)
    ...
    for color_name, impression_name in assignments.items():
        ...
        color_possible_values = [ColorCandidate(impression_name=c, score=s, reason=r) for c, s, r in candidates]
...
def _three_round_assign(
    ...
    high_conf_pairs = [
        (mat[ci, ii], ci, ii) for ci in range(n_colors) for ii in range(n_imps) if mat[ci, ii] >= HIGH_CONF_THRESHOLD
    ]
