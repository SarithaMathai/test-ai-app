...
def _load_latest_extended_eval() -> dict[str, Any] | None:
    """Load the most recent extended evaluation run."""
    db = get_db()
    if db is None:
        return None
    try:
        doc = db[_EXTENDED_EVAL_COL].find_one(sort=[("created_at", -1)])
        return doc
    except Exception:
        return None
...
def _load_eval_history(limit: int = 10) -> list[dict]:
    """Load recent evaluation runs for trend analysis."""
    db = get_db()
    if db is None:
        return []
    try:
        docs = list(
            db[_EXTENDED_EVAL_COL]
            .find({})
            .sort("created_at", -1)
            .limit(limit)
        )
...
def _render_overview():
    ...
    if not eval_run:
        st.info("No evaluation data available. Run POST /api/v1/eval/detailed to generate metrics.")
        return
    ...
    with col5:
        st.metric(
            ...
            delta="↓ Better (Well-calibrated)" if eval_run.get('confidence_calibration_error', 1.0) < 0.10 else "↑ Worse",
        )
    ...
    with col1:
        ...
        st.metric(
            ...
            f"{high_correction:.1%}",
            delta="↓ Better (should be <15%)" if high_correction < 0.15 else "❌ Issues" if high_correction > 0.25 else "✓ OK",
        )
...
def _render_per_signal_analysis():
    ...
    st.markdown(
        "How accurate is each scoring signal? "
        "Higher correction_rate = weaker signal."
    )
    ...
    for col, (signal_type, metrics) in zip(cols, per_signal.items()):
...
def _render_per_department_analysis():
    ...
    st.markdown(
        "Accuracy varies by product department. "
        "Identify departments that need focused improvement."
    )
    ...
    for col, metric in zip(cols, per_dept):
...
def _render_llm_impact():
    ...
    st.markdown(
        "Compare LLM-assisted matching vs pure deterministic to see if LLM is helping."
    )
    ...
    if improvement > 0.10:
        ...
        st.info(
            f"✓ **LLM is moderately helpful** ({improvement:+.1%} improvement). "
