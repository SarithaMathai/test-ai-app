...
from plm_tcin_mapper.ui.utils import confidence_badge, size_sort_key
...
def _impression_options(pid: str, db) -> list[str]:
    try:
        return sorted(db.variation_records.distinct("impression_name", {"pid": pid}))
    except Exception:
        return []


def _save_correction(m: dict, impression_name: str, db) -> bool:
    try:
        mapping_id = str(m.get("_id", ""))
        pid = m.get("pid", "")
        var = db.variation_records.find_one(
            {"pid": pid, "impression_name": impression_name},
            {"impression_id": 1},
        )
        impression_id = var.get("impression_id") if var else None
        fb = FeedbackRecord(
            ...
            suggested_impression_id=impression_id,
        ...
        db.feedback.insert_one(fb.model_dump(by_alias=True))
        db.mappings.update_one(
            {"_id": mapping_id},
            {"$set": {
                "status": "CORRECTED",
                "matched_impression_name": impression_name,
                "matched_impression_id": impression_id,
                "updated_at": datetime.now(UTC),
            }},
        )
        return True
    except Exception as exc:
        st.error(f"Error saving correction: {exc}")
        return False


def _clear_impression(m: dict, db) -> bool:
    try:
        mapping_id = str(m.get("_id", ""))
        db.mappings.update_one(
            {"_id": mapping_id},
        ...
        st.error(f"Error clearing impression: {exc}")
        return False
    ...
    for color_key, color_maps in color_groups.items():
        ...
        if chosen == _NULL_SENTINEL:
            if original:  # only clear if there was something to clear
                for m in color_maps:
                    if _clear_impression(m, db):
                        cleared += 1
        elif chosen != original:
            for m in color_maps:
                if chosen != (m.get("matched_impression_name") or "") and _save_correction(m, chosen, db):
                    saved += 1
...
def _competing_candidates_html(m: dict) -> str:
    ...
    competing = [
        ...
        if top_score - c.get("score", 0) <= _CANDIDATE_GAP and c.get("impression_name")
    ]
    if len(competing) < 2:
        return ""
    badges = []
    for c in competing:
        ...
        score = c.get("score", 0)
        bg, fg = ("#d1f0d1", "#1a5c1a") if score >= 0.85 else (("#fff3cd", "#7d5a00") if score >= 0.60 else ("#fde8e8", "#8b1a1a"))
        badges.append(
    ...
    return (
        '<span style="color:#aaa;font-size:0.73em;margin-right:4px">⚖️ debating:</span>'
        + "".join(badges)
    )
...
def _render_color_row(
    ...
    if review_open:
        ...
        c3.markdown(
            confidence_badge(confidence) if not is_no_match
        ...
        conf_cell = (
            '<span style="color:#aaa;font-size:0.82em">—</span>' if is_human
            else (confidence_badge(confidence) if not is_no_match
                  else '<span style="color:#ccc;font-size:0.82em">—</span>')
        )
        ...
        st.markdown(
            ...
            + (f'<div style="font-size:0.75em;color:#aaa;padding:2px 0 4px 0">'
               f'<span style="color:#ccc">Sizes: </span>{"  ·  ".join(size_parts)}</div>' if size_parts else "")
            + '</div>',
            unsafe_allow_html=True,
        )
...
def render_pid_card(
    ...
    if toast_data:
        msg, ok = toast_data
        st.toast(msg, icon="✅" if ok else "ℹ️")  # noqa: RUF001

    st.markdown("")

    # ── Impression options ─────────────────────────────────────────────────────
    impression_options = sorted({v.get("impression_name", "") for v in var_docs if v.get("impression_name")})
    if not impression_options and db is not None:
        impression_options = _impression_options(pid, db)
    ...
    for color_key, color_maps in sorted(color_groups.items(), key=_sort_groups):
        _render_color_row(
            pid, color_key, color_maps, impression_options,
            key_suffix, review_open, originals, col_w,
        )

    # ── Save / Cancel footer (only when review is open) ────────────────────────
    if review_open and db is not None:
        st.markdown("")
        ...
        f1.button(
            ...
            args=(pid, mapping_docs, key_suffix, db),
        )
        f2.button(
            "Cancel",
...
def render() -> None:
    ...
    with st.spinner(f"Loading {pid} …"):
        mapping_docs = list(db.mappings.find({"pid": pid}).sort("tcin_id", 1))
        var_docs = list(db.variation_records.find({"pid": pid}))
