...
def render():
    ...
    st.markdown(
        "Automated analysis of evaluation metrics recommending configuration improvements."
    )
...
def _render_pending_analysis():
    ...
    st.markdown(
        "Analyze the latest evaluation run to generate threshold adjustment proposals."
    )
...
def _render_pending_review():
    ...
    st.markdown(
        "These proposals are ready for human review. Click to see details and preview impact."
    )
    ...
    for idx, proposal in enumerate(proposals):
        with st.container(border=True):
            ...
            with col1:
                ...
                st.markdown(
                    f"{_confidence_color(confidence)} **{proposal_type}** "
    ...
    for proposal in applied:
        with st.container(border=True):
            ...
            for change in changes:
                ...
                st.markdown(
                    f"• `{param}`: `{current:.4f}` → `{proposed:.4f}`"
                )
