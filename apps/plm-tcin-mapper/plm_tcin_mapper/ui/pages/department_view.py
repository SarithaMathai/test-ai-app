...
def _load_pids_for_dept(dept_id: str, pid_filter: str, limit: int = 50) -> list[dict]:
    ...
    try:
        ...
        for d in db.mappings.aggregate(pipeline):
            ...
            needs_review = any(
                s in ("NO_MATCH", "NEEDS_REVIEW", "NEEDS_SPOT_CHECK") for s in statuses
            )
            rows.append({
                "pid": d["_id"],
                "total": d["total"],
...
def _batch_load_mappings(dept_id: str, pids: tuple[str, ...]) -> dict[str, list[dict]]:
    ...
    try:
        docs = list(
            db.mappings.find({"pid": {"$in": list(pids)}}).sort([("pid", 1), ("tcin_id", 1)])
        )
...
def render() -> None:
    ...
    overridden_count = sum(
        1 for r in pid_rows
        if bool(st.session_state.get("pid_overrides", {}).get(r["pid"]))
    )
