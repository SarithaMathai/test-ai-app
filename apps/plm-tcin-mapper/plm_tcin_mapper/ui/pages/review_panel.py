...
from plm_tcin_mapper.database.models import FeedbackAction, FeedbackRecord, MappingStatus
...
_REVIEW_STATUS_MAP = {
    ...
    "All Pending":    ["NO_MATCH", "NEEDS_REVIEW", "NEEDS_SPOT_CHECK"],
}
...
def _load_queue(
    ...
    if db is None:
        return []
    statuses = _REVIEW_STATUS_MAP.get(queue_type, ["NEEDS_REVIEW"])
    filt: dict = {"status": {"$in": statuses}}
    if match_rounds:
        filt["match_round"] = {"$in": match_rounds}
    return list(
        db.mappings
        .find(filt)
        .sort("created_at", 1)
...
def _submit_feedback(
    ...
    """Write FeedbackRecord and update Mapping status. Returns True on success."""
    db = get_db()
    if db is None:
        return False
    ...
    if action == FeedbackAction.CORRECT and suggested_impression:
        var = db.variation_records.find_one(
            {"pid": pid, "impression_name": suggested_impression},
            {"impression_id": 1},
        )
        if var:
            suggested_id = var.get("impression_id")
    ...
    fb = FeedbackRecord(
        ...
        suggested_impression_id=suggested_id,
    ...
    try:
        ...
        new_status = _STATUS_AFTER_ACTION[action]
        update: dict = {
            "$set": {
        ...
        if action == FeedbackAction.CORRECT and suggested_impression:
            update["$set"]["matched_impression_name"] = suggested_impression
            if suggested_id:
                update["$set"]["matched_impression_id"] = suggested_id

        db.mappings.update_one({"_id": mapping_id}, update)
        return True
...
def render():
    ...
    remaining = [m for m in queue if str(m.get("_id", "")) not in reviewed]
    ...
    if not remaining:
        if not queue:
            ...
            st.success(
                f"All {len(reviewed)} items reviewed this session. "
                "Click **Load Queue** to fetch fresh items."
            )
        return
    ...
    with nav_info:
        st.markdown(
            ...
            f"Card <b>{idx + 1}</b> of <b>{len(remaining)}</b></div>",
            unsafe_allow_html=True,
        )
    with nav_next:
...
def _render_no_match_card(m: dict, reviewer: str, reviewed: set):
    ...
    with conf_col:
        st.metric("Confidence", f"{score:.0%}", delta=f"{tier} tier", delta_color="off",
...
def _render_review_card(m: dict, reviewer: str, reviewed: set):
    ...
    if sorted_cands:
        ...
        for c in sorted_cands[:5]:
            ...
            st.markdown(
                f"  {c_icon} **{c.get('impression_name', '?')}** "
                f"— {c_score:.0%}  ·  {c.get('reason', '')}"
            )
