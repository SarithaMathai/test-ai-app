...
def render():
    ...
    st.markdown(
        "Review and apply keyword refinement proposals from corrected feedback."
    )
...
def _render_pending_proposals():
    ...
    st.markdown(
        "These proposals are ready for human review. "
        "Click on a proposal to see details and decide whether to approve."
    )
        ...
        with st.container(border=True):
            ...
            with col1:
                ...
                if proposal_type == "ALIAS_MOVE":
                    st.markdown(
                        f"**Move `{keyword}` from `{base_color}` → `{suggested_color}`**"
                    )
                else:
...
def _render_approved_proposals():
    ...
    if approved:
        ...
        for idx, proposal in enumerate(approved):
            with st.container(border=True):
                ...
                st.caption(f"Confidence: {proposal.get('confidence', 0):.1%} | Impact: {proposal.get('frequency', 0)} mappings")
    ...
    if applied:
        ...
        for proposal in applied:
            ...
            st.markdown(
                f"✅ **{keyword}** (`{base}` → `{target}`) — Applied at {applied_at}"
            )
