...
from plm_tcin_mapper.database.models import ShadowComparisonResult, ShadowMetricComparison
from plm_tcin_mapper.models.schemas import ShadowComparisonResponse
...
class ShadowComparator:
    ...
    def _compare_sync(self, baseline_batch_id: str, shadow_batch_id: str) -> ShadowComparisonResponse:
        ...
        recommendation = self._generate_recommendation(
            metric_comparisons, overall_improvement, is_significant
        )
        ...
        result = ShadowComparisonResult(
            ...
            correction_rate_improvement=self._get_metric_improvement(
                metric_comparisons, "correction_rate"
            ),
    ...
    def _compute_metrics(self, mappings: list[Any]) -> dict[str, float]:
        ...
        from plm_tcin_mapper.database.models import ConfidenceTier, MappingStatus
    ...
    def _compare_metrics(
        self, baseline: dict[str, float], shadow: dict[str, float]
    ) -> list[ShadowMetricComparison]:
    ...
    def _get_metric_improvement(
        comparisons: list[ShadowMetricComparison], metric_name: str
    ) -> float:
    ...
    def _calculate_p_value(baseline_mappings: list[Any], shadow_mappings: list[Any]) -> float:
        ...
        """
        from plm_tcin_mapper.database.models import MappingStatus
