...
def run_extended_eval(db: Any, persist: bool = True) -> ExtendedEvalRun:
    ...
    high_confidence_correction_rate = (
        round(high_confidence_corrections / high_confidence_total, 4)
        if high_confidence_total > 0
        else 0.0
    )
    low_confidence_correction_rate = (
        round(low_confidence_corrections / low_confidence_total, 4)
...
def _compute_per_signal_accuracy(
    ...
    for signal_type, stats in signal_stats.items():
        ...
        avg_confidence = (
            round(stats["confidence_sum"] / occurrences, 4)
            if occurrences > 0
            else 0.0
        )
...
def _compute_per_department_metrics(
    ...
    dept_stats: dict[str, dict] = defaultdict(lambda: {
        "total": 0,
        "high_confidence": 0,
        "corrections": 0,
        "confidence_sum": 0.0,
...
def _compute_llm_impact(
    ...
    deterministic_correction_rate = (
        round(deterministic_corrected / deterministic_total, 4)
...
def _compute_calibration_error(
    ...
    """
    bins: dict[int, dict] = {
        i: {"confidence_sum": 0.0, "count": 0, "corrections": 0}
        for i in range(10)
    }
