...
from plm_tcin_mapper.database.models import ConfidenceTier, EvalRun, MappingStatus
...
def run_eval(db: Any, cfg: Any, persist: bool = True) -> EvalRun:
    ...
    if pct(n_high) < eval_cfg.min_high_confidence_pct:
        alerts.append(
            f"LOW HIGH-CONFIDENCE RATE: {pct(n_high):.1%} "
            f"(threshold: {eval_cfg.min_high_confidence_pct:.0%})"
        )

    if pct(n_low) > eval_cfg.max_low_confidence_pct:
        alerts.append(
            f"HIGH LOW-CONFIDENCE RATE: {pct(n_low):.1%} "
            f"(threshold: {eval_cfg.max_low_confidence_pct:.0%})"
        )
