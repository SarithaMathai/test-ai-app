...
class ThresholdTuner:
    ...
    def _generate_proposals(self, eval_run: ExtendedEvalRun, eval_id: str) -> list[ThresholdProposal]:
        ...
        if correction_rate > 0.30:
            proposal = self._propose_lower_auto_confirm_threshold(
                eval_run, eval_id, supporting_metrics
            )
            if proposal:
                proposals.append(proposal)

        if pct_high < 0.35:
            proposal = self._propose_raise_llm_threshold(
                eval_run, eval_id, supporting_metrics
            )
            if proposal:
                proposals.append(proposal)
        ...
        if eval_run.llm_impact and eval_run.llm_impact.llm_correction_rate > 0.20:
            proposal = self._propose_adjust_llm_fallback(
                eval_run, eval_id, supporting_metrics
            )
            if proposal:
                proposals.append(proposal)
        ...
        if correction_rate > 0.25 and pct_high > 0.40:
            proposal = self._propose_fuzzy_weight_reduction(
                eval_run, eval_id, supporting_metrics
            )
            if proposal:
                proposals.append(proposal)
        ...
        if eval_run.confidence_calibration_error > 0.15:
            proposal = self._propose_confidence_recalibration(
                eval_run, eval_id, supporting_metrics
            )
            if proposal:
                proposals.append(proposal)
