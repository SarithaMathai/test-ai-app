...
def get_pids_for_department(department: str) -> list[dict[str, Any]]:
    """Get per-PID summary rows for a department (for department view page)."""
    result = get("/api/v1/mappings/pids", department=department)
    return result.get("pids", [])
...
def get_eval_detailed_latest() -> dict[str, Any] | None:
    """Get latest detailed evaluation results. Returns None if no eval has been run yet."""
    with _get_client() as client:
        response = client.get("/api/v1/eval/detailed/latest")
        response.raise_for_status()
