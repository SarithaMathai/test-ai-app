...
from plm_tcin_mapper_client.utils import confidence_badge, confidence_tier_badge, size_sort_key
...
def _render_color_row(
    ...
    if review_open:
        ...
        c3.markdown(
            (
                confidence_badge(confidence) + confidence_tier_badge(m0.get("confidence_tier", ""))
                if not is_no_match
        ...
        # Confidence cell: numeric score + tier badge
        tier = m0.get("confidence_tier", "")
        if is_human:
            conf_cell = '<span style="color:#2563eb;font-size:0.78em;font-weight:600">👤 human</span>'
        elif is_no_match:
            conf_cell = '<span style="color:#ccc;font-size:0.82em">—</span>'
        else:
            conf_cell = confidence_badge(confidence)
            if tier:
                conf_cell += f"<br>{confidence_tier_badge(tier)}"
