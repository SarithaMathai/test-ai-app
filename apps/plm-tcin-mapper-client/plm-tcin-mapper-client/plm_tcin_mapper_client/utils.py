...
def confidence_tier_badge(tier: str) -> str:
    """HTML pill for confidence tier label (HIGH / GOOD / FAIR / LOW)."""
    icon, bg, fg = _TIER_STYLE.get(tier.upper(), ("⚪", "#f3f4f6", "#6b7280"))
    return (
        f'<span style="background:{bg};color:{fg};padding:2px 7px;'
        f"border-radius:12px;font-size:0.74em;font-weight:700;"
        f'display:inline-block;margin-left:4px">{icon} {tier}</span>'
    )
